...
def fun_with_ids2
  ...
  puts
#this will still cause an error since all the inner variables are only accessible inside the scope of an_illustrative_method
...
#A and D will be the same since the object ID of the value of 42 does not change. I think B and C will change object ID since it creates new array/string to point to
  ...
  c_outer = [44]
  d_outer = c_outer[0]
#they should all have new object ids after reassignment
  puts "a_outer inside after reassignment is #{a_outer} with an id of: #{a_outer_id} before and: #{a_outer.object_id} after."
  puts "b_outer inside after reassignment is #{b_outer} with an id of: #{b_outer_id} before and: #{b_outer.object_id} after."
  puts "c_outer inside after reassignment is #{c_outer} with an id of: #{c_outer_id} before and: #{c_outer.object_id} after."
  puts "d_outer inside after reassignment is #{d_outer} with an id of: #{d_outer_id} before and: #{d_outer.object_id} after."
...
#could use a ternary to do the same thing
def color_valid(color)
